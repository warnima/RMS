...
<html>
    <head>
        ...
        <title>NDB BANK | Manual Entry CW Auth</title>
    ...
    <script type="text/javascript">
        ...
        %>
            ...
            $("#idndb_customer_define_seller_id").change(function () {
                ...
                if (!(idndb_customer_define_seller_id === 'Select The Seller')) {
                    ...
                    getUnauthorizedCwPdcData(idndb_customer_define_seller_id);
            ...
            function getUnauthorizedCwPdcData(idndb_customer_define_seller_id) {
                $("#loading-mask").show();
                ...
                var values = "&idndb_customer_define_seller_id=" + idndb_customer_define_seller_id +
                        "&pdc_product=CW&pdc_report_type=MANUAL&paramid=" + paramid;
                $.ajax({
                    type: "POST",
            ...
            $("#msrt_save_pdccw_auth_data").click(function (e) {
            ...
            $("#msrt_save_pdccw_reject_data").click(function (e) {
        ...
        function getUnAuthTxnsSellerDetails() {
            ...
            var values = "&pdc_product=CW&pdc_manual_bulk=MANUAL&paramid=" + paramid;
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if ((Object.keys(data).length) === 0) {
                        $("#loading-mask").hide();
                        swal("No data found !", "Un-Authorized CW transactions not available in th system. !");
                    } else {
        ...
        function updatePdcCwAsAuthorized() {
            $("#loading-mask").show();
            ...
            document.getElementById("msrt_save_pdccw_reject_data").disabled = true;
            document.getElementById("msrt_save_pdccw_auth_data").disabled = true;
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.success) {
                        window.location = "modalsSucess.jsp?desigURL=ndb_pdc_cw_manual_authorization.jsp&message=PDC cheque warehousing entries authorized successfully !"


                    } else if (data.systemna) {
                        window.location = "modalsError.jsp?desigURL=ndb_pdc_cw_manual_authorization.jsp&message=" + data.systemna + "";

                    } else if (data.error) {
                        window.location = "modalsError.jsp?desigURL=ndb_pdc_cw_manual_authorization.jsp&message=Error occured in authorizing PDC cheque warehousing entry date  !"
        ...
        function updatePdcCwAsRejected() {
            $("#loading-mask").show();

            var paramid = "msrt_save_pdccw_reject_data";
            $('input[type="submit"]').prop('disabled', false);
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.success) {
                        window.location = "modalsSucess.jsp?desigURL=ndb_pdc_cw_manual_authorization.jsp&message=PDC cheque warehousing entries rejected successfully !";
                        ...
                        window.location = "modalsError.jsp?desigURL=ndb_pdc_cw_manual_authorization.jsp&message=Error occured in rejecting PDC cheque warehousing date  !";
        ...
        function fnExcelReport() {
            ...
            if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) {
                if (window.navigator.msSaveBlob) {
                    ...
                    navigator.msSaveBlob(blob, 'UN_AUTHORIZED_CW_PDC_DATA.xls');
                }
            } else {
                $('#test').attr('href', data_type + ', ' + encodeURIComponent(tab_text));
                $('#test').attr('download', 'UN_AUTHORIZED_CW_' + idndb_customer_define_seller_id + '.xls');
            }
        ...
        <div class="wrapper">
            ...
            <div class="content-wrapper">
                ...
                <section class="content-header">
                    <h1>
                        Manual Entry CW PDC Authorization
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">Manual Entry CW PDC Authorization</li>
                    </ol>
                </section>
...
//                                                        }
                                ...
                                <div class="box-footer">
                                    <button type="submit" class="btn btn-info pull-right" id="msrt_save_pdccw_reject_data" style="width: 90px;">Rejected</button>
                                    <button type="submit" class="btn btn-info pull-right" id="msrt_save_pdccw_auth_data" style="margin-right: 5px;width: 90px;">Authorized</button>
                        ...
                        <div class="col-xs-12" style="margin-top: auto;">
                            <div class="box">
                                <div class="box-header">
                                    <h3 class="box-title">Manual Entry UN-Authorized CW PDC Data</h3>
                                    ...
                                    <table id="example1" class="table table-bordered table-striped" style="cursor: pointer;">
                                        ...
                                        <tbody>
                                            <%                                                if (_session_availability) {
                                                    ...
                                                    //out.print(new FIllDataTableBean().getCWPDCUnAuthData(session.getAttribute("userid").toString()));
                                                }
                                            %>
                                        </tbody>
                                        <tfoot>
