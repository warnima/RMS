...
<html>
    <head>
        ...
        <title>NDB BANK | Form Master</title>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#ndb_user_rms").change(function () {
                if (this.checked) {
                    $("#ndb_user_poms").prop('checked', false);
                    $("#ndb_user_um").prop('checked', false);
                }
            });

            $("#ndb_user_poms").change(function () {
                if (this.checked) {
                    $("#ndb_user_rms").prop('checked', false);
                    $("#ndb_user_um").prop('checked', false);
                }
            });

            $("#ndb_user_um").change(function () {
                if (this.checked) {
                    $("#ndb_user_rms").prop('checked', false);
                    $("#ndb_user_poms").prop('checked', false);
                }
            ...
            $("#btn_form_save").click(function (e) {


                if (validateInput()) {

                    var paramid = "msrt_save_form_data";
                    var values = "&idndb_user_forms=" + $("#idndb_user_forms").val().trim() +
                            "&ndb_form_name=" + $("#ndb_form_name").val().trim() +
                            "&ndb_form_path=" + $("#ndb_form_path").val().trim() +
                            "&ndb_form_main_menu_id=" + $("#ndb_form_main_menu_id").val().trim() +
                            "&ndb_form_oder_by=" + $("#ndb_form_oder_by").val().trim() +
                            "&ndb_form_status=" + ($("#ndb_form_status").prop('checked') ? "ACTIVE" : "DEACTIVE") +
                            "&ndb_user_rms=" + ($("#ndb_user_rms").prop('checked') ? "RMSACTIVE" : "DEACTIVE") +
                            "&ndb_user_poms=" + ($("#ndb_user_poms").prop('checked') ? "POMSACTIVE" : "DEACTIVE") +
                            "&ndb_user_um=" + ($("#ndb_user_um").prop('checked') ? "UMACTIVE" : "DEACTIVE") +
                            "&paramid=" + paramid;
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.success) {
                                window.location = "modalsSucess.jsp?desigURL=ndb_forms_file.jsp&message=Form data saved to DB successfuly.";

                            } else if (data.error) {
                                window.location = "modalsError.jsp?desigURL=ndb_forms_file.jsp&message=Error occured in saving form data.";
            ...
            $("#example1 tbody tr").on("click", function (event) {
                ...
                var idndb_user_forms = $(this).find("td").eq(0).html();
                var paramid = "get_form_data_tofill";
                var values = "&idndb_user_forms=" + idndb_user_forms +
                        "&paramid=" + paramid;
                $.ajax({
                    type: "POST",
                    ...
                    success: function (data) {
                        $.each(data, function (k, v) {
                            $('#idndb_user_forms').val(v.idndb_user_forms);
                            $('#ndb_form_name').val(v.ndb_form_name);
                            $("#ndb_form_path").val(v.ndb_form_path);
                            $("#ndb_form_main_menu_id").val(v.ndb_form_main_menu_id);
                            $("#ndb_form_oder_by").val(v.ndb_form_oder_by);

                            if (v.ndb_form_status === 'ACTIVE') {
                                $("#ndb_form_status").prop('checked', true);
                            }

                            if (v.ndb_user_system_group === 'RMSACTIVE') {
                                $("#ndb_user_rms").prop('checked', true);
                                $("#ndb_user_poms").prop('checked', false);
                                $("#ndb_user_um").prop('checked', false);
                            }

                            if (v.ndb_user_system_group === 'POMSACTIVE') {
                                $("#ndb_user_poms").prop('checked', true);
                                $("#ndb_user_rms").prop('checked', false);
                                $("#ndb_user_um").prop('checked', false);
                            }

                            if (v.ndb_user_system_group === 'UMACTIVE') {
            ...
            function validateInput() {
                ...
                var hasError = true;
                var ndb_form_name = $("#ndb_form_name").val();
                var ndb_form_path = $("#ndb_form_path").val();
                var ndb_form_main_menu_id = $("#ndb_form_main_menu_id").val();
                var ndb_form_oder_by = $("#ndb_form_oder_by").val();

                if (ndb_form_name === '') {
                    $('#ndb_form_name').css('border-color', 'red');
                    return false;
                }
                if (ndb_form_path === '') {
                    $('#ndb_form_path').css('border-color', 'red');
                    return false;
                }
                if (ndb_form_main_menu_id === 'Select The Form Attached Menu') {
                    $('#ndb_form_main_menu_id').css('border-color', 'red');
                    return false;
                }
                if (ndb_form_oder_by === '') {
                    $('#ndb_form_oder_by').css('border-color', 'red');
                    return false;
                }
            ...
            <div class="content-wrapper">
                ...
                <section class="content-header">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">Form Master File</li>
                    </ol>
                </section>
                        ...
                        <div class="col-xs-12">
                            <div class="box">
                                <form class="form-horizontal" id="branch_data" name="branch_data">
                                    <div class="box-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control" id="ndb_form_name" name="ndb_form_name" style="width: 350px">
                                                <input type="hidden" class="form-control" id="idndb_user_forms" name="idndb_user_forms" style="width: 350px">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label class="col-sm-2 control-label">Form Path</label>
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control" id="ndb_form_path" name="ndb_form_path" style="width: 350px">
                                            </div>
                                        </div>

                                        <div class="form-group">
                                            <label class="col-sm-2 control-label">Menu Attached System</label>
                                            <div class="col-sm-10">
                                                <input type="checkbox" id="ndb_user_rms">PDC AND RF System
                                                <input type="checkbox" id="ndb_user_poms">POMS System
                                                <input type="checkbox" id="ndb_user_um">User Management System
                                            </div>
                                        </div>

                                        <div class="form-group">
                                            <label class="col-sm-2 control-label">Form Attached Menu</label>
                                            <div class="col-sm-10">

                                                <select class="form-control" id="ndb_form_main_menu_id" style="width: 350px;">
                                                    <option>Select The Form Attached Menu</option>    
                                                    <%                                                        out.print(new FillDataComboBean().getFormAttachedMenusDataCmb(session.getAttribute("userid").toString()));
                                        ...
                                        <div class="form-group">
                                            <label class="col-sm-2 control-label">Oder By</label>
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control" id="ndb_form_oder_by" name="ndb_form_oder_by" style="width: 350px">
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-10">
                                                <input type="checkbox" id="ndb_form_status">Click to Active
                                            </div>
                                        </div>
                                ...
                                <div class="box-footer">
                                    ...
                                    <button type="submit" class="btn btn-info pull-right" id="btn_form_save" name="btn_branch_save">Submit</button>
                                </div><!-- /.box-footer -->
                            </div>
                        </div>
                    </div>
                                    ...
                                    <table id="example1" style="cursor: pointer;" class="table table-bordered table-striped">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Form Name</th>
                                                <th>Form Path</th>
                                                <th>Form Attached System</th>
                                                <th>Form Attached ID</th>
                                                <th>Form Order By</th>
                                                <th>Form Status</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <%                                                            if ((session.getAttribute("userid") == null) || (session.getAttribute("userid") == "")) {
                                                    ...
                                                    out.print(new FIllDataTableBean().getUserFormsData(session.getAttribute("userid").toString()));
                                                }
                                            %>
                                        </tbody>
                                        <tfoot>
