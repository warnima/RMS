...
<%@page import="DBAutoFillBeans.AutoFillBean"%>
...
<html>
    <head>
        ...
        <title>NDB BANK | Customer Master</title>
    ...
    <script type="text/javascript">
        ...
        <% if (user_type.equals("12") || user_type.equals("9") || user_type.equals("10") || user_type.equals("11")) { %>
                ...
                document.getElementById("btn_cutomerdfine_save").disabled = true;
        ...
        <%}  %>
            ...
            $("#cust_short_name").keyup(function () {
                if (this.value.match(/[^a-zA-Z0-9 ]/g)) {
                    this.value = this.value.replace(/[^a-zA-Z0-9-/,.(/)/: / ]/g, '');
                }
            });
            ...
            $("#cms_curr_acc_number").keyup(function () {
                if (this.value.match(/[^0-9 ]/g)) {
                    this.value = this.value.replace(/[^0-9]/g, '');
                }
            });
            $("#cms_collection_acc_number").keyup(function () {
                if (this.value.match(/[^0-9 ]/g)) {
                    this.value = this.value.replace(/[^0-9]/g, '');
                }
            });
            ...
            $("#btn_cutomerdfine_save").click(function (e) {


                if (validateInput()) {
                    $("#loading-mask").show();
                    $("#btn_cutomerdfine_save").prop("disabled", true);
                    var statusReason = ($("#statusReason").val() !== '') ? $("#statusReason").val().trim() : "";
                    var paramid = "msrt_save_cust_define_data";
                    var values = "&cust_id=" + $("#cust_id").val().trim() + "&cust_name=" + $("#cust_name").val().trim() +
                            "&idndb_customer_define=" + $("#idndb_customer_define").val().trim() +
                            "&cust_short_name=" + $("#cust_short_name").val().trim() +
                            "&cust_indutry_id=" + $("#cust_indutry_id").val().trim() +
                            "&cust_contct_no=" + $("#cust_contct_no").val().trim() +
                            "&cust_fx_number=" + $("#cust_fx_number").val().trim() +
                            "&cust_contct_per1=" + $("#cust_contct_per1").val().trim() +
                            "&cust_contct_per2=" + $("#cust_contct_per2").val().trim() +
                            "&cust_email=" + $("#cust_email").val().trim() +
                            "&cust_address=" + $("#cust_address").val().trim() +
                            "&cust_rec_acc_no=" + $("#cust_rec_acc_no").val().trim() +
                            "&cust_cre_des_pros_to_acc_no=" + $("#cust_cre_des_pros_to_acc_no").val().trim() +
                            "&cust_curr_ac_no=" + $("#cust_curr_ac_no").val().trim() +
                            "&cms_curr_acc_number=" + $("#cms_curr_acc_number").val().trim() +
                            "&cms_collection_acc_number=" + $("#cms_collection_acc_number").val().trim() +
                            "&cust_margin_ac_no=" + $("#cust_margin_ac_no").val().trim() +
                            "&cust_margin=" + $("#cust_margin").val().trim() +
                            "&cust_credt_rate=" + $("#cust_credt_rate").val().trim() +
                            "&cust_bank=" + $("#cust_bank").val().trim() +
                            "&cust_branch=" + $("#cust_branch").val().trim() +
                            "&cust_other_bank_ac_no=" + $("#cust_other_bank_ac_no").val().trim() +
                            "&cust_geo_market=" + $("#cust_geo_market").val().trim() +
                            "&cust_ch_active=" + ($("#cust_ch_active").prop('checked') ? "ACTIVE" : "DEACTIVE") +
                            "&statusReason=" + statusReason +
                            "&cust_ndb_customer_active=" + ($("#cust_ndb_customer_active").prop('checked') ? "ACTIVE" : "DEACTIVE") +
                            "&paramid=" + paramid;
                    $.ajax({
                        type: "POST",
                        ...
                        success: function (data) {
                            //  alert("ok");
                            if (data.success) {
                                window.location = "modalsSucess.jsp?desigURL=ndb_customer_master.jsp&message=Customer define data saved to DB successfuly and sent for authorization.";
                            } else if (data.systemna) {
                                window.location = "modalsError.jsp?desigURL=ndb_customer_master.jsp&message=" + data.systemna + "";

                            } else if (data.alreadyexdata) {
                                window.location = "modalsError.jsp?desigURL=ndb_customer_master.jsp&message=Error occured in saving customer define data. Customer ID already exist.";
                            } else if (data.cannotamendunauthdat) {
                                window.location = "modalsError.jsp?desigURL=ndb_customer_master.jsp&message=" + data.cannotamendunauthdat + "";
                            } else if (data.error) {
                                window.location = "modalsError.jsp?desigURL=ndb_customer_master.jsp&message=Error occured in saving customer define data.";
                            }
                        }
                    });
                }
            ...
            $("#btn_view_customer_data").click(function (e) {
                var search_criteria = $("#search_criteria").val();
                window.location = '/NDBRMS/pages/MasterFiles/ndb_customer_master.jsp?table_plus=true&search_criteria=' + search_criteria + '';



            });

            $('#btn_cutomerdfine_clear').click(function () {
                location.reload();
            });
            ...
            $("#cust_id").focusout(function () {
                ...
                var paramid = "get_customer_data_tofill_USING_CUST_ID";
                var values = "&cust_id=" + cust_id +
                        "&paramid=" + paramid;
                $.ajax({
                    type: "POST",
                    url: "/NDBRMS/CustomerDataRetrive",
            ...
            function validateInput() {
                ...
                var _customer_type = "NOT DEFINE";
                if ($("#cust_ndb_customer_active").prop('checked')) {
                    _customer_type = "NDB_CUST"
                } else {
                    _customer_type = "NON_CUST"

                }

                if (_customer_type === 'NON_CUST') {
                    if (cust_id === '') {
                    ...
                    //  if (!(cust_email === '')) {
                    //
                    //                      var atpos = cust_email.trim().indexOf("@");
                    //                    var dotpos = cust_email.trim().lastIndexOf(".");
                ...
                if (!($("#cust_ch_active").prop('checked'))) {
                    if ($("#statusReason").val() === '') {
        ...
        function fnExcelReport() {
            ...
            if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) {
                if (window.navigator.msSaveBlob) {
                    ...
                    navigator.msSaveBlob(blob, 'CUSTOMER DATA.xls');
                }
            } else {
                $('#test').attr('href', data_type + ', ' + encodeURIComponent(tab_text));
                $('#test').attr('download', 'CUSTOMER DATA.xls');
            }
            ...
            <div class="content-wrapper">
                ...
                <section class="content-header">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">Define Customer</li>
                    </ol>
                </section>
                        ...
                        <div class="col-xs-12">
                                    ...
                                    <div class="box-body">
                                        ...
                                        <div id="_ndb_customer">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-10">
                                                    <input type="email" class="form-control" id="cust_email" name="cust_email" style="width: 350px">
                                                </div>
                                            </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-10">
                                                    <input type="text" class="form-control" id="cust_margin" name="cust_margin" placeholder="0.00"  value="0" style="width: 350px" onchange="handleChange(this);">
                                                </div>
                                            </div>
                                            <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-1">
                                                <input type="checkbox" id="cust_ch_active">Click to Active
                                            </div>
                            ...
                            <div class="box box-info">                   
                                <div class="box-footer">
                                    ...
                                    <button type="submit" class="btn btn-info pull-right" id="btn_cutomerdfine_clear" style="margin-top: -33px;">Cancel</button>
                                    <button type="submit" class="btn btn-info pull-right" id="btn_cutomerdfine_save" style="margin-right: 5px;margin-top: -33px;">Submit</button>
                                    <button type="submit" class="btn btn-info pull-right" id="btn_view_customer_data" style="margin-top: -34px;margin-right: 314px;">View Customer Define Data</button>
                                </div>
                            </div>
                        ...
                        <div class="col-xs-12">
                            <div class="box">
                                <div class="box-header">
                                    <h3 class="box-title">Customer Data Report :</h3>
                                    ...
                                    <table id="example1" style="cursor: pointer;" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            <%   if (_session_availability) {
                                                    ...
                                                    out.print(new FIllDataTableBean().getCustomerDifineData(search_criteria));
